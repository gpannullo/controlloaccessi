...
class DirectoryAdminService(ActiveDirectoryService):
    ...
    def dettaglio(self, username):
        ...
        try:
            ...
            return {
                ...
                "active": not bool(int(entry.userAccountControl.value or 0) & 2),
    ...
    def crea_utente(self, username, first_name, last_name, email, personal_email, mobile, password):
        self._require_writes()
        if not settings.DIRECTORY.get("USE_SSL", False):
            raise DirectoryAdminException("La creazione con password richiede LDAPS.")
        safe_cn = " ".join(item for item in [first_name.strip(), last_name.strip()] if item) or username.strip()
        dn = "CN=%s,%s" % (safe_cn, settings.DIRECTORY["USER_SEARCH_BASE"])
        conn = self._connection()
        try:
            ...
            attributes = {
                ...
                "userPrincipalName": "%s@%s" % (username.strip(), settings.DIRECTORY["DOMAIN"]),
                "givenName": first_name.strip(),
                "sn": last_name.strip(),
                "displayName": safe_cn,
